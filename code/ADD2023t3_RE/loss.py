...
class ArcMarginProduct(nn.Module):
    def __init__(self, in_features=160, out_features=2, s=30.0, m=0.7, sub=1, easy_margin=False):
...
class OCSoftmax(nn.Module):
    ...
    def forward(self, x, labels):
        ...
        scores = x @ w.transpose(0, 1)
        output_scores = scores.clone()
        #print(scores[labels == 0][0],"scores[labels == 0]")
        #print(scores[labels == 1][0],"scores[labels == 1]")
        scores[labels == 0] = self.r_real - scores[labels == 0]
